...
# file_name = 'az+16'
base_dir = '/home/michael/Documents/data/astro/032022/2703/sun/'
file_name = 'az+04'
...
jc[0:10] = np.zeros(jc.shape[1])

# jc[20] = nans(jc.shape[1])
# jc[46] = nans(jc.shape[1])

# jc[51:56] = nans(jc.shape[1])

# jc[71] = nans(jc.shape[1])

jc[76] = nans(jc.shape[1])
jc[102] = nans(jc.shape[1])

# jc[107] = nans(jc.shape[1])
jc[112] = nans(jc.shape[1])
# jc[117] = nans(jc.shape[1])
jc[122] = nans(jc.shape[1])
jc[127] = nans(jc.shape[1])

jc[133] = nans(jc.shape[1])

# jc[138] = nans(jc.shape[1])
# jc[143] = nans(jc.shape[1])
# jc[153] = nans(jc.shape[1])
# jc[158] = nans(jc.shape[1])

jc[194] = nans(jc.shape[1])
jc[199] = nans(jc.shape[1])


jc[204] = nans(jc.shape[1])
jc[209] = nans(jc.shape[1])
jc[332] = nans(jc.shape[1])
jc[373] = nans(jc.shape[1])


jc[387:402] = np.zeros(jc.shape[1])
# jc[449] = nans(jc.shape[1])

jc[470:523] = np.zeros(jc.shape[1])

# jc[558] = nans(jc.shape[1])
# jc[643] = nans(jc.shape[1])
# jc[644] = nans(jc.shape[1])
# jc[788] = nans(jc.shape[1])

jc[1006:1024] = np.zeros(jc.shape[1])
...
for i in range(0, loadings.shape[1]):
    fig.add_trace(go.Scatter(y=(1 if i==0 else 5) * np.flip(loadings[:, i]), line=dict(width=4, color=colors[i])))
fig.show()
...
jc = np.clip(jc, a_min=0, a_max=30)
...
#     fig.add_trace(go.Scatter(y=np.flip(jc[i]), line=dict(color=colors[i])))
# fig.show()
# exit()
